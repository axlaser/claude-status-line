...
state_json=$(printf '%s' "$input" | jq -c '{tasks: [(.tasks // [])[] | select(type == "object") | {id, name, type, description, status, model, effort, contextWindowSize, tokenCount, startTime} | with_entries(select(.value != null))]}' 2>/dev/null)
[[ -z "$state_json" ]] && exit 0
