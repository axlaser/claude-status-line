...
# Compatibility entry point. The real uninstaller is install${action}.ps1; this
...
if ($_local -and (Test-Path $_local)) {
    ...
    try {
        $_body = (Invoke-WebRequest -UseBasicParsing -ErrorAction Stop `
            -Uri "https://raw.githubusercontent.com/axlaser/claude-statusline/master/install/uninstall.ps1").Content
    } catch {
        Write-Host "  Could not fetch install/uninstall.ps1" -ForegroundColor Red
        return
    }
