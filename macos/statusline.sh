...
if [[ -d "$git_dir" ]]; then
    head_mt=$(stat -f %m "${git_dir}/HEAD" 2>/dev/null || echo 0)
    index_mt=$(stat -f %m "${git_dir}/index" 2>/dev/null || echo 0)
    cwd_mt=$(stat -f %m "${git_cwd}" 2>/dev/null || echo 0)
    git_mt_key="${head_mt}|${index_mt}|${cwd_mt}"

    git_cache_path=""
    [[ -n "$session_id" ]] && git_cache_path="${TMPDIR:-/tmp}/statusline-git-${session_id}.txt"

    if [[ -n "$git_cache_path" && -f "$git_cache_path" ]]; then
        IFS=$'\t' read -r cached_key cached_branch cached_ins cached_del cached_unt < "$git_cache_path"
        if [[ "$cached_key" == "$git_mt_key" ]]; then
            branch="$cached_branch"
            insertions="$cached_ins"
            deletions="$cached_del"
            untracked="$cached_unt"
            use_git_cache=true
        fi
    fi
    ...
    if [[ "$use_git_cache" != true ]]; then
        ...
        if [[ -n "$branch" ]]; then
            ...
            if [[ -n "$git_cache_path" ]]; then
                printf '%s\t%s\t%s\t%s\t%s' "$git_mt_key" "$branch" "$insertions" "$deletions" "$untracked" > "$git_cache_path" 2>/dev/null
            fi
        fi
    fi
fi
...
if [[ -n "$transcript_path" && -f "$transcript_path" ]]; then
    ...
    if [[ "$use_cache" != true ]]; then
        if [[ -s "$transcript_path" ]]; then
            ...
            total_user=$(grep -cE '"type"[[:space:]]*:[[:space:]]*"user"' "$transcript_path" 2>/dev/null || echo 0)
            tool_results=$(grep -c '"toolUseResult"' "$transcript_path" 2>/dev/null || echo 0)
            meta_users=$(grep -cE '"isMeta"[[:space:]]*:[[:space:]]*true' "$transcript_path" 2>/dev/null || echo 0)
            slash_users=$(grep -c '<command-name>' "$transcript_path" 2>/dev/null || echo 0)
...
# Args: label, value, delta, idle_color, active_color
