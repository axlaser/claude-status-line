...
# deliberately per-platform (BSD `-f '%N %m'` here, GNU `-c '%n %Y'` on Linux).
...
if (( ${#_oc_stat_paths[@]} > 0 )); then
    ...
    done < <(stat -f '%N %m' "${_oc_stat_paths[@]}" 2>/dev/null)
...
read -r _oc_key _ < <(printf '%s' "${raw}|${_oc_tmt}|${_oc_gmt}|${_oc_smt}|${_oc_ffresh}|${_oc_fjson}|${_oc_mwmt}|$(( _oc_now / 5 ))" | shasum -a 256 2>/dev/null)
