#!/usr/bin/env bash
# Compatibility entry point. The real installer is install/install.sh, which
...
if [[ -n $_SELF_DIR && -f "$_SELF_DIR/../install/install.sh" ]]; then
    # Running from a clone.
    source "$_SELF_DIR/../install/install.sh"
else
    ...
    # script.
    _BODY=$(curl -fsSL "https://raw.githubusercontent.com/axlaser/claude-statusline/master/install/install.sh")
    if [[ -z $_BODY ]]; then
        printf '  Could not fetch install/install.sh\n' >&2
